...
!在IDT第8和128(0x80)项分别设置定时器中断门描述符和系统调用陷阱门描述符
        ...
        movw    $oxef00,%dx               !陷阱门type=15,特权级为3也可以用
        movl    $0x80,%ecx
...
!现在开始转到任务0(任务A)来执行操作堆栈内容，欺骗CPU进行中断返回到特权级3的用户空间
        ...
        pushfl                  !eflags入栈
        pushf   $0x0f           !任务0当前局部空间代码段选择子入栈
        pushf   $task0          !任务0代码指针入栈
...
write_char:
        ...
        pushl   %ebx
        mov     $SRN_SEL,%ebx     !然后让GS指向显存段(0xb800)
        movl    %bx, %gs
        movl    scr_loc,%bx       !再从变量src_loc取得当前字符的显示位置值
...
.align 2   !注意>>>老版本的编译器为2的n次方，新版本的将直接给出对齐值
ignore_int:
        push    %ds
        pushl   %eax
...
!*******************************************************************
current:.long 0             !全局变量，当前任务号码(0或者1)
src_loc:.long 0             !屏幕当前显示位置，按从左上角到右下角
