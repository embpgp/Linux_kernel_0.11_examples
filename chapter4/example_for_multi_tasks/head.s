...
#在新的位置重新布置IDT和GDT
        ...
        mov     %ax,%fs
        mov     %ax,%gs
        lss     init_stack,%esp  #同时加载esp和ss,低32位送esp,高16位送ss
...
#由于汇编语言极其容易出错，导致我刚刚开始跑起来之后一直处在中断处理里面
#就是一直不断地输出C，最后直接崩溃，后来一步一步查看才得知栈空间不平衡
setup_idt:
        lea     ignore_int,%edx
        movl    $0x00080000,%eax
        movw    %dx,%ax
...
果然，第一次是直接彰显了，而后面的每一次都会造成一定的"误导"，因此通过这个调试，解决了我的疑惑，同时我也
更加深刻地领会了Intel芯片的复杂性，其实更复杂的软件工程师都略过好多了.....

2016.11.29:
关于理解书中讲到的tss状态段中的tss.ss0和tss.esp0在进行任务切换的时候不会被保存，因此对于
CPU来讲就是"只读"的，因此每次任务进入内核态执行代码的时候栈空间总是原来的(因为esp0没有被修改)
之前一直未理解清除，参照TSS状态段格式可知总体分为两类字段
1.动态字段，当任务切换被挂起的时候，处理器会自动更新(CPU自动取得当前任务的tr寄存器，找到tss段的位置，将自身的寄存器等上下文内容
　保存过去以便于恢复)，包括
 (1)通用寄存器eax,ecx,edx,ebx,esp,ebp,esi,edi(即pushad指令的顺序)
 (2)段选择符ES,CS,SS,DS,FS,GS
 (3)EFLAGS
 (4)EIP
 (5)先前任务链接字段
 因此，保存了这些东西后处理器才去加载新的tss内容，此时所有的东西都是新的内容了，包括了下面的静态字段，否则怎么切换环境呢？
2.静态字段，处理器只会读取其内容，仅仅只在切换到新任务的时候被装入CPU，其值在被创建的时候就被"锁定"，
　不由CPU自动修改，当然可以通过程序猿手动编程修改的，包括
 (1)LDT
 (2)CR3
 (3)特权级的0、1、2堆栈指针字段，包括了SS0-2和ESP0-2
 (4)调试陷阱T字段(tss状态段的偏移第0x64比特处)，用于切换到下一个任务也产生调试异常
 (5)I/O图基地址字段

观察到中断处理函数结束的时候iret指令才终于明白真的不改变，iret指令仅仅将栈上的EIP,CS,EFLAGS,SS和ESP重新装入
中断前的任务足以恢复了，中断空间还是属于当前任务空间，因此可以持续的(其实在iret之前有一大波寄存器被pop了)。
*/
